...
The suite is 114 tests across 13 files, run with `pnpm test` (Vitest). It runs in about 1.6 s. The strategy is to test the pure cores directly and to spawn the real processes where a process boundary is the thing that could break.
...
| `tests/cross-ide.test.ts` | budget config load/save/sanitise and percentage→fraction conversion; the MCP session id derived from the client; lesson distillation (recurring topic across sessions, min-count filter, ignoring structural path segments) |
...
| `tests/plugin-validate.test.ts` | the whole plugin validates: manifest, MCP declaration, statusline, marketplace, all seven hooks, commands, agents, output style, 59 skills |
