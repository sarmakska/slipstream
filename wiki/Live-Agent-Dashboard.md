...
`src/dashboard/server.ts`, built on `node:http` with no framework. It binds `127.0.0.1` (port 0 lets the OS pick a free port), serves the self-contained UI at `/`, exposes `/api/sessions` and `/api/state`, and streams `/api/stream` as server-sent events. A poll loop tails the active session logs and pushes a fresh `state` frame whenever the log grows.
