...
- `src/mcp` (`server.ts`, `tools.ts`, `index.ts`). The bundled MCP server. `tools.ts` defines the twelve `sp_` tools and `callTool`, each a thin wrapper over the same library the CLI uses; `server.ts` is the pure `handleRequest` dispatcher plus the newline-delimited JSON-RPC stdio loop; `index.ts` is the entry point Claude Code spawns. See [MCP tools](MCP-Tools).
...
- `src/memory` (`store.ts`, `types.ts`, `recall.ts`, `digest.ts`, `embed.ts`, `observe.ts`, `search.ts`). The persistent memory store and the features built on it. `store.ts` is one Markdown file per fact under `.claude/slipstream/memory/` with a regenerated `MEMORY.md` index (`addMemory`, `pruneMemory`, `listMemories`, `recallMemories`, `regenerateIndex`). `recall.ts` is signal-ranked recall (`rankBySignal`, `selectRelevant`); `digest.ts` builds the PreCompact session digest (`buildDigest`, `digestToMemory`). `embed.ts` is the local hashed term-frequency embedding (`embed`, `cosine`); `observe.ts` folds the event log into the observation store under `.claude/slipstream/observations/` (`foldObservations`, `captureObservations`, `loadObservations`, `getObservations`); `search.ts` is the three-layer hybrid search (`searchObservations`, `timeline`, plus `rankObservations`). See [Memory recall](Memory-Recall), [Lossless compaction](Lossless-Compaction) and [Observation memory and semantic search](Observation-Memory).
...
- `src/cli` (`index.ts`, `skills-dir.ts`). The helper entry point. It dispatches `map`, `slice`, `lines`, `guard`, `budget`, `memory` (including `search`, `timeline`, `observations`), `observe`, `mindmap`, `status`, `dashboard`, `validate`, `plugin-validate`, `doctor`, `statusline`, `recall-signal` and `digest`.

## Data that lives in your project

slipstream writes only under `.claude/slipstream/` in your project: `map.md` and `map.json`; `memory/` with one file per fact plus `MEMORY.md`; `observations/` with one append-only `<session>.jsonl` of auto-captured observations, a per-session `.cursor` and a project `.counter`; `dashboard/` with one append-only `<session>.jsonl` event log per session and a `server.json` recording the running server; and an optional `dashboard.json` for settings. This directory is git-ignored by default in this repo and is meant to be local to each developer unless you choose to commit the memory.
