...
- `src/cli` (`index.ts`, `skills-dir.ts`). The helper entry point. It dispatches `map`, `slice`, `lines`, `guard`, `budget`, `memory`, `mindmap`, `status`, `dashboard`, `validate` and `plugin-validate`.
...
- **The helper owns only the objective parts** (indexing, retrieval, budget, validation, event recording). The judgement (what to remember, when to compact, whether a gate is the right one) stays with the agent following the skills.
