...
- The bundled MCP server (`cp_map`, `cp_symbol`, `cp_lines`, `cp_search`, `cp_remember`/`cp_recall`/`cp_forget`, `cp_budget`, `cp_mindmap`), so Claude works through precise tools. See [MCP tools](MCP-Tools).
...
- Three shipped subagents (cp-shipper, cp-schema, cp-reviewer). See [Subagents](Subagents).
- A context-budget statusline and a terse output style. See [Statusline](Statusline) and [Output style](Output-Style).
- `/claudepilot:doctor`, an end-to-end install check.
