...
claudepilot tracks an approximate context budget from the bytes pulled into context and converts it to a token estimate against the model window. This is honest guidance, not a guaranteed counter, but it lets you compact early rather than discovering the limit the hard way.

## Steps

1. Estimate the bytes you have read this session (file reads, slices, command output). A rough figure is fine.
2. Run `npx claudepilot status . --bytes <bytes>` to see the budget level (`ok`, `warn` or `compact`) and its advice, alongside the project mind map.
