...
1. To start (or reuse) the dashboard and print its localhost URL, run `npx claudepilot dashboard start .`. Start is idempotent, so this reuses a running server rather than spawning another.
...
3. To list recorded sessions, run `npx claudepilot dashboard sessions .`. To replay one, run `npx claudepilot dashboard replay . --session <id>` and report the reconstructed agent count, statuses and approximate tokens.
4. If the browser did not open, the URL in the chat is the fallback. To disable auto-open, set `autoOpen` to false in `.claude/claudepilot/dashboard.json` or export `CLAUDEPILOT_DASHBOARD_OPEN=0`.
